...
Maps every one of the 119 Python `test_*` functions across the five named
source files (`tests/reference/test_primitives.py`, `test_engine.py`,
`test_relations.py`, `test_oracles.py`, `test_evaluate.py`) to its
TypeScript coverage. No function is silently omitted.
...
- **(b) corpus/matrix parity** -- additionally (never as a substitute for
  ...
  checks C1-C4, 924 records total, IDs discarded and reassigned
...
Counts: 118 of 119 functions have a direct (a) TypeScript test; 2 of those
118 additionally carry (b) corpus/matrix parity coverage; 1 function is
(c) Python-only-justified with no TypeScript counterpart.

## tests/reference/test_primitives.py -> reference/ts/test/primitives.test.ts (62/62 direct)
