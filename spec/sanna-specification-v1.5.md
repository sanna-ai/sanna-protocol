...
Sanna is trust infrastructure for AI agents. This document specifies
the Sanna Reasoning Receipt format, the Constitution policy format,
the canonical fingerprint construction algorithm, the canonicalization
rules for cross-implementation byte parity, the Ed25519 signing
protocol, and the verification protocol.
...
| `original_hash` | string | Yes | The SHA-256 hex digest of the Unicode Normalization Form C (NFC)-normalized original value. Format: `[a-f0-9]{64}` (64 lowercase hexadecimal characters, bare digest with no prefix). |
...
Identifies the governance event. The prefix indicates the execution surface. Unprefixed values are MCP (gateway) surface events, consistent with GCD Layer 4 (Enforcement) event types.

| Value | Surface | Meaning |
|-------|---------|---------|
| `invocation_allowed` | MCP | Tool invocation permitted |
| `invocation_halted` | MCP | Tool invocation blocked |
| `invocation_escalated` | MCP | Tool invocation escalated for approval |
| `cli_invocation_allowed` | CLI | Subprocess invocation permitted |
| `cli_invocation_halted` | CLI | Subprocess invocation blocked |
| `cli_invocation_escalated` | CLI | Subprocess invocation escalated |
...
| `invocation_modified` | MCP | Tool invocation permitted with parameter transformation applied (Agentic Action Risk Management (AARM) MODIFY) |
| `invocation_deferred` | MCP | Tool invocation deferred pending additional context (AARM DEFER) |
| `cli_invocation_anomaly` | CLI | Agent attempted invocation of a non-delivered CLI pattern (only when authority_boundaries.anomaly_tracking.cli is true) |
...
| `api_invocation_anomaly` | API | Agent attempted invocation of a non-delivered HTTP pattern (only when authority_boundaries.anomaly_tracking.http is true) |
...
**Anomaly tracking opt-in (normative, v1.5+):**

`cli_invocation_anomaly` and `api_invocation_anomaly` receipts are emitted
ONLY when the constitution's `authority_boundaries.anomaly_tracking`
field enables the respective surface:

```yaml
authority_boundaries:
  anomaly_tracking:
    cli: true    # enables cli_invocation_anomaly emission
    http: true   # enables api_invocation_anomaly emission
```

When `anomaly_tracking` is absent or the surface flag is false (the
default), the interceptor suppresses commands/endpoints per the
`session_manifest` but does NOT emit anomaly receipts for blocked
attempts. The MCP gateway always emits `invocation_anomaly` regardless
of this field (MCP anomaly tracking is unconditional).

The `anomaly_tracking` field participates in constitution hashing ONLY
when non-default (at least one surface is true). Constitutions without
`anomaly_tracking` or with both surfaces false hash identically to
constitutions created before this field existed. No re-signing required.
...
Documents what the governance boundary can observe about the action lifecycle. This is critical for verifiers: the value determines how `input_hash` and `action_hash` should be interpreted.

| Value | Meaning |
|-------|---------|
| `gateway_boundary` | MCP gateway. Cannot observe downstream tool execution. `action_hash == input_hash`. |
| `cli_execution` | CLI interceptor. Observes full subprocess lifecycle. `action_hash` computed from execution output. |
| `cli_no_justification` | CLI interceptor, no justification provided. `reasoning_hash` is SHA-256 of empty string. |
| `api_execution` | HTTP interceptor. Observes full request/response lifecycle. `action_hash` computed from response. |
| `api_no_justification` | HTTP interceptor, no justification provided. `reasoning_hash` is SHA-256 of empty string. |

`context_limitation` does NOT participate in fingerprint computation.
...
**Example (informative):** A Go SDK implementation would register `"sanna-go"` via spec PR before emitting receipts with that value. Until registered, `tool_name: "sanna-go"` fails schema validation.
...
**Example (normative, absent vs null):**
...
| 8 | `coverage_hash` | `hash_obj()` of `evaluation_coverage` object. `EMPTY_HASH` if absent/null/empty. |
| 9 | `authority_hash` | `hash_obj()` of `authority_decisions` array. `EMPTY_HASH` if absent/null/empty. |
| 10 | `escalation_hash` | `hash_obj()` of `escalation_events` array. `EMPTY_HASH` if absent/null/empty. |
| 11 | `trust_hash` | `hash_obj()` of `source_trust_evaluations` array. `EMPTY_HASH` if absent/null/empty. |
...
Note: Fields 1 and 4 contribute their literal string values to the
fingerprint formula. All other components contribute 64-character
hexadecimal SHA-256 strings or `EMPTY_HASH`.

**Empty semantics:** For fields 7-12, implementations treat `[]`
...
**v1.2 correction:** The formula documented in v1.1 incorrectly listed
`spec_version`, `tool_version`, `timestamp`, `agent_id` (correlation_id),
`query_hash`, and `status` as fingerprint fields, and omitted
`enforcement_hash`, `coverage_hash`, `authority_hash`, `escalation_hash`,
...
- `spec_version` (not part of fingerprint formula)
- `tool_version` (not part of fingerprint formula)
- `timestamp` (not part of fingerprint formula)
- `status` (not part of fingerprint formula)
- `receipt_id` (random)
...
Example PEM header/footer (informative):
...
| `authority_boundaries` | Cannot/must/can execute lists (MCP surface) |
| `cli_permissions` | CLI subprocess governance rules (v1.2+) |
| `api_permissions` | HTTP/API governance rules (v1.2+) |
...
### 6.6 Multi-Surface Governance Blocks (v1.2)

`cli_permissions` and `api_permissions` are OPTIONAL top-level
constitution blocks that extend governance beyond MCP tool invocations.

- `authority_boundaries` governs MCP tool invocations (gateway surface).
- `cli_permissions` governs subprocess invocations (CLI surface).
...
### 7.6 CLI Execution Boundary

When the Receipt Triad is produced by a CLI subprocess interceptor (`context_limitation: "cli_execution"` or `"cli_no_justification"`), the triad fields have different semantics than at the gateway boundary.

**`input_hash`:**

The input object for CLI invocations captures the command, arguments, working directory, and environment variable names:
...
input_obj = {
    ...
    "env_keys": sorted_environment_variable_names
}
input_hash = SHA-256(canonical_json_bytes(input_obj))
```

Where:
- `command` is the resolved binary name (e.g., `"git"`, `"curl"`), not the full path.
- `args` is the array of argument strings (e.g., `["push", "origin", "main"]`).
...
The action object for CLI invocations captures the execution output:
...
action_hash = SHA-256(canonical_json_bytes(action_obj))
```

Where:
- `exit_code` is an integer, or `null` if the process was halted before spawning.
- `stdout` and `stderr` are UTF-8 strings. Binary output MUST be base64-encoded before inclusion.
- For halted invocations (process never spawned): `{"exit_code": null, "stderr": "", "stdout": ""}`.
- Large outputs MAY be truncated at a configurable limit. The hash is computed over the truncated content.

Canonical JSON key ordering: `exit_code`, `stderr`, `stdout`.

**At the CLI boundary, `action_hash != input_hash`.** The CLI interceptor observes the full execution lifecycle.

### 7.7 API Execution Boundary

When the Receipt Triad is produced by an HTTP interceptor (`context_limitation: "api_execution"` or `"api_no_justification"`), the triad fields capture the request/response lifecycle.

**`input_hash`:**

```text
input_obj = {
    "body_hash": sha256_of_request_body_or_EMPTY_HASH,
    "headers_keys": sorted_request_header_names,
    "method": http_method_uppercase,
...
- `headers_keys` is a sorted (lexicographic, case-sensitive) array of request header names. Values are excluded (contain auth tokens).
- `body_hash` is the 64-hex SHA-256 of the raw request body bytes, or `EMPTY_HASH` if no body.

Canonical JSON key ordering: `body_hash`, `headers_keys`, `method`, `url`.
...
```text
action_obj = {
    "body_hash": sha256_of_response_body_or_EMPTY_HASH,
    "response_headers_keys": sorted_response_header_names,
    "status_code": integer_or_null
}
action_hash = SHA-256(canonical_json_bytes(action_obj))
```

Where:
- `status_code` is the HTTP status code integer, or `null` if the request was halted before sending.
- `response_headers_keys` is a sorted array of response header names. Values excluded.
- `body_hash` is the 64-hex SHA-256 of the raw response body bytes, or `EMPTY_HASH` if no body.
- For halted invocations: `{"body_hash": EMPTY_HASH, "response_headers_keys": [], "status_code": null}`.

Canonical JSON key ordering: `body_hash`, `response_headers_keys`, `status_code`.

**At the API boundary, `action_hash != input_hash`.**
...
**Example (informative) -- verifying a receipt set:**
