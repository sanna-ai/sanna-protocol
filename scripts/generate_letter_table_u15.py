#!/usr/bin/env python3
"""SAN-896 (spec erratum e14, operator-ratified): generates
reference/ts/src/unicode_letters_v15.ts, a table of maximal contiguous
code-point ranges where Unicode General Category is one of
{Lu, Ll, Lt, Lm, Lo} under CPython 3.12 / Unicode Character Database
(UCD) version 15.0.0.
...
NORMATIVE STATUS: spec section 2.2 rule 4 / erratum e14 (SAN-896) pins
"letters" to Unicode General Category {Lu, Ll, Lt, Lm, Lo} under UCD
15.0.0. This generator/table implements that spec-level pin for
TypeScript at the CPython 3.12 / UCD 15.0.0 reference/CI baseline: the
predicate below (is_letter()) is the category query the spec
normatively requires -- the same category query reference/primitives.py's
_is_letter() performs for Python. On this CPython 3.12 / UCD 15.0.0
baseline the category-based predicate is exhaustively verified
equivalent to chr(cp).isalpha() over every code point in
[0, 0x10FFFF]: the generated range table is byte-identical to the one
this generator produced before the SAN-896 predicate rewrite.
...
def is_letter(cp: int) -> bool:
    """unicodedata.category(chr(cp)) in {Lu, Ll, Lt, Lm, Lo} IS the
    provenance for this table (spec section 2.2 rule 4 / erratum e14,
    SAN-896): the generated ranges are defined as exactly the code
    points where this category predicate is True under CPython 3.12 /
    UCD 15.0.0. Surrogate code points (U+D800-U+DFFF), which CPython can
    represent as lone-surrogate strings but which are never valid
    Unicode scalar values, are forced to False explicitly rather than
    relying on unicodedata.category() behavior over them (their category
    is Cs, which is not in _LETTER_CATEGORIES, so the explicit guard is
...
def render(ranges: list[tuple[int, int]]) -> str:
    ...
    lines.append("// includes cp if and only if unicodedata.category(chr(cp)) is one of")
    lines.append('// {"Lu", "Ll", "Lt", "Lm", "Lo"} under CPython 3.12 / Unicode Character')
    lines.append("// Database (UCD) version 15.0.0 -- that category predicate call IS the")
    lines.append("// provenance, not a description of one (spec section 2.2 rule 4 /")
    lines.append("// erratum e14 LETTER_v1, SAN-896). Lone surrogate code points")
    lines.append("// (U+D800-U+DFFF) are forced to False. On this baseline the category")
    lines.append("// predicate is exhaustively verified equivalent to chr(cp).isalpha():")
    lines.append("// this table is byte-identical to the one produced by the prior")
    lines.append("// isalpha()-based predicate.")
    lines.append("//")
    ...
    lines.append("//")
    lines.append("// NORMATIVE STATUS: spec section 2.2 rule 4 / erratum e14 (SAN-896,")
    lines.append('// operator-ratified) pins "letters" to Unicode General Category')
    lines.append("// {Lu, Ll, Lt, Lm, Lo} under UCD 15.0.0. This table implements that")
    lines.append("// spec-level pin for TypeScript at the CPython 3.12 / UCD 15.0.0")
    lines.append("// reference/CI baseline.")
    lines.append("")
